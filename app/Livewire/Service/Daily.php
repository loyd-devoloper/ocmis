...
class Daily extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        // Fetch UserService data filtered by status and date range
        $top = UserService::with('category')
            ->where('status', \App\Enums\StatusEnum::Paid->value)
        ...
        foreach ($top as $key => $v) {
            ...
            $labels[] = $key;
            $values[] = $v->sum('price'); // Assuming 'total_paid' is the field you want to sum
        }
        // Initialize date range for the last 30 days or the selected range
        ...
        //     $values[] = $top->get($formattedDate, collect())->sum('price'); // Sum prices for the corresponding date/group
